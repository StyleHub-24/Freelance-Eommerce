...
const ProductItem = ({ id, name, colorVariants, bestseller }) => {
  ...
  const currentPrice = colorVariants?.[selectedColorIndex]?.price;
  
  // Calculate total stock for the selected color variant
  const currentColorStock = colorVariants?.[selectedColorIndex]?.sizes?.reduce((total, size) => total + size.stock, 0) || 0;
    ...
    <Link className='text-gray-700 cursor-pointer relative group' to={`/product/${id}`}>
          ...
          {colorVariants?.map((variant, index) => {
            ...
            return (
              <button
                ...
                } transition-all hover:scale-110 ${totalStock === 0 ? 'opacity-50' : ''}`}
                style={{ backgroundColor: variant.color }}
                onClick={(e) => {
                  e.preventDefault();
                  setSelectedColorIndex(index);
                }}
                aria-label={`Select ${variant.color} color${totalStock === 0 ? ' (Out of Stock)' : ''}`}
              />
            );
          })}
        </div>
      ...
      <div className='mt-2'>
        ...
        <div className="flex justify-between items-center">
          <p className='text-sm font-medium'>{currency}{currentPrice}</p>
