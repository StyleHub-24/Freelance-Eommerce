...
const Product = () => {
  ...
  const fetchProductData = () => {
    ...
    if (product && product.colorVariants && product.colorVariants.length > 0) {
      ...
      // If urlColor exists, find and select that color variant, otherwise use the first one
      const colorVariant = urlColor
        ? product.colorVariants.find((variant) => variant.color === urlColor) ||
  ...
  const handleColorChange = (colorVariant) => {
    ...
    // Update URL with color parameter from color selection
    const currentSearchParams = new URLSearchParams(window.location.search);
    currentSearchParams.set("color", colorVariant.color);
  ...
  const getCurrentSizeStock = () => {
    ...
    const sizeData = selectedColorVariant.sizes.find(
  ...
  const isOutOfStock =
    ...
    selectedColorVariant.sizes.every((size) => size.stock === 0);
      ...
      <div className="flex gap-12 sm:gap-12 flex-col sm:flex-row">
        ...
        <div className="flex-1 flex flex-col-reverse gap-3 sm:flex-row">
          ...
          <div className="w-full sm:w-[80%] relative">
            <img
              className={`w-full h-auto ${isOutOfStock ? "opacity-50" : ""}`}
              src={image}
              alt=""
            />
            {isOutOfStock && (
        ...
        <div className="flex-1">
          ...
          <p className="mt-5 text-3xl font-medium">
            {currency} {selectedColorVariant?.price}
          </p>
          {isOutOfStock && (
            <div className="mt-4 bg-red-50 border border-red-200 rounded-md p-4">
              <p className="text-red-700 font-medium">Currently Out of Stock</p>
              <p className="text-red-600 text-sm mt-1">
                This item in {selectedColorVariant?.color} is temporarily
                unavailable
              </p>
            </div>
          )}
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      outline: isLightColor(variant.color)
                        ? "1px solid #ddd"
                        : "none",
                    }}
